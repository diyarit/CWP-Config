...
yum erase yum-cron -y || dnf erase yum-cron -y 2>/dev/null || true

echo "Removing Polkit..."
yum erase polkit -y || dnf erase polkit -y 2>/dev/null || true
...
if [ -d /usr/local/cwpsrv/ ]; then
	...
	sleep 15
		cd /usr/local/src; wget https://centos-webpanel.com/cwp-el$ver-latest; sh cwp-el$ver-latest
	...
	systemctl enable ip6tables
 	...
 	echo "deactivating Firewalld..."
        ...
        if [ -f /proc/vz/veinfo ] && grep -i "release 8" /etc/redhat-release > /dev/null; then # IN AL8 AND OPENVZ CSF DOES NOT WORK WITH THE NEW IPTABLES, AN OLDER VERSION OF CENTOS 7 IS INSTALLED
                ...
                yum install https://vault.centos.org/centos/7/os/x86_64/Packages/iptables-1.4.21-35.el7.x86_64.rpm -y
...
sed -i 's/^LF_EMAIL_ALERT = .*/LF_EMAIL_ALERT = "1"/g' /etc/csf/csf.conf
...
sed -i 's/^LT_EMAIL_ALERT = .*/LT_EMAIL_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^RT_RELAY_ALERT = .*/RT_RELAY_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^RT_AUTHRELAY_ALERT = .*/RT_AUTHRELAY_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^RT_POPRELAY_ALERT = .*/RT_POPRELAY_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^RT_LOCALRELAY_ALERT = .*/RT_LOCALRELAY_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^RT_LOCALHOSTRELAY_ALERT = .*/RT_LOCALHOSTRELAY_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^CT_EMAIL_ALERT = .*/CT_EMAIL_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^PT_USERKILL_ALERT = .*/PT_USERKILL_ALERT = "1"/g' /etc/csf/csf.conf
sed -i 's/^PS_EMAIL_ALERT = .*/PS_EMAIL_ALERT = "1"/g' /etc/csf/csf.conf
...
mysql --defaults-file=/root/.my.cnf root_cwp -e "UPDATE backups SET backup_enable = 'off' WHERE id='1'" || true
...
sh /scripts/install_cbpolicyd || true
...
find /usr/local/php/ -name "php.ini" | xargs sed -i 's/^date.timezone.*/date.timezone = "America\/Chicago"/g'
...
systemctl stop clamd || true
systemctl disable clamd || true
yum remove clamav* -y || true
